<template>
  <div class="space-y-6">
    <!-- Family Header -->
    <div class="bg-white/90 dark:bg-stone-800/90 backdrop-blur-sm border-b-2 border-orange-200 dark:border-stone-700">
      <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div class="flex items-center gap-3">
            <div
              class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-50 to-rose-50 dark:from-orange-900/20 dark:to-rose-900/20 rounded-full shadow-sm border-2 border-orange-200 dark:border-orange-700">
          ...
          <div class="text-sm font-bold text-gray-700 bg-white/80 px-3 py-1.5 rounded-full border border-gray-200">
    ...
    <main class="max-w-7xl mx-auto px-4 py-8 space-y-8 pb-24 md:pb-8">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <div class="text-center lg:text-left">
          ...
          <p class="text-xl text-gray-600 dark:text-gray-300 font-medium max-w-2xl">
            Private messages to
            {{ authStore.currentFamilyName || "family" }} members for the future
          </p>
        </div>
        <NuxtLink v-if="authStore.isAuthenticated && authStore.hasFamily" to="/capsules/create"
      ...
      <div v-if="!authStore.isAuthenticated" class="text-center py-12">
        ...
        <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">Sign In Required</h3>
        <p class="text-gray-600 dark:text-gray-300 text-lg mb-8 max-w-md mx-auto font-medium">
      ...
      <!-- Loading State -->
      <div v-else-if="loading && capsules.length === 0" class="text-center py-12">
      ...
      <!-- Error State -->
      <div v-else-if="capsulesError"
        class="bg-gradient-to-r from-red-50 to-rose-50 border-2 border-red-300 rounded-3xl p-6">
        <div class="flex items-start gap-4">
          <i class="fas fa-exclamation-triangle text-red-500 text-2xl flex-shrink-0 mt-1"></i>
          <div class="flex-1">
      ...
      <div v-else class="space-y-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <div
            class="bg-white/90 dark:bg-stone-800/90 backdrop-blur-sm rounded-3xl shadow-lg border-2 border-blue-200 dark:border-blue-700 p-6 hover:shadow-xl transition-all transform hover:-translate-y-1">
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="text-3xl font-bold text-gray-900 dark:text-white">{{ sentCount }}</p>
                <p class="text-gray-600 dark:text-gray-300 font-bold">Sent</p>
              </div>
            </div>
          </div>
        ...
        <div
          class="bg-white/90 dark:bg-stone-800/90 backdrop-blur-sm rounded-3xl shadow-xl border-2 border-orange-200 dark:border-stone-700 p-6 md:p-8">
          <!-- Mobile Dropdown -->
          <div class="lg:hidden mb-6">
            <label class="block text-lg font-bold text-gray-700 mb-3">Filter by</label>
            <div class="relative">
          ...
          <!-- Capsules Grid -->
          <div v-if="filteredCapsules.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <CapsuleCard v-for="capsule in filteredCapsules" :key="capsule.id" :capsule="capsule" @view="viewCapsule"
          ...
          <div v-else class="text-center py-12">
            ...
            <NuxtLink v-if="showCreateButton" to="/capsules/create"
              class="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-orange-500 to-rose-600 text-white font-bold rounded-2xl hover:from-orange-600 hover:to-rose-700 transition-all duration-200 hover:shadow-2xl transform hover:-translate-y-1 shadow-lg">
              <i class="fas fa-plus"></i>
        ...
        <!-- How It Works -->
        <div
          class="bg-white/90 dark:bg-stone-800/90 backdrop-blur-sm rounded-3xl shadow-lg border-2 border-blue-200 dark:border-blue-700 p-8">
